-- AdverX: post-day-7 earning rating curve.
-- First 7 days stay at 100% normal reward.
-- From day 8, earning rating declines 2.5% per day, floored at 50%.
-- Daily reward limits remain hard ceilings, not payout targets.

ALTER TABLE public.user_plans
  ADD COLUMN IF NOT EXISTS earning_rating_pct numeric NOT NULL DEFAULT 100.00,
  ADD COLUMN IF NOT EXISTS earning_rating_updated_at timestamptz;
...
BEGIN
  ...
  v_earning_rating := CASE
    ...
    ELSE greatest(50.00,
      round(100.00 * power(0.975::numeric, v_plan_age_days - 7), 2))
  END;
  ...
  ELSE
    v_reward := round(
      least(v_remaining_daily, coalesce(p.max_ad_reward_pkr, v_remaining_daily))
      * v_decay_multiplier, 2
    );
  END IF;
  ...
  IF v_reward > 0 THEN
    ...
    VALUES(v_user, 'ad_reward', v_reward, s.id,
