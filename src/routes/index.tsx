  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section className="border-b border-border/70 bg-muted/20">
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 pb-16 pt-12 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:pb-24 lg:pt-20">
          <div>
            ...
            <h1 className="max-w-2xl text-balance text-5xl font-bold leading-[0.98] tracking-[-0.05em] sm:text-6xl lg:text-[4.5rem]">
              {hero.headline}
            </h1>
      ...
      <HowItWorks />
      <section id="workspace" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28">
...
function PublicHeader() {
  return (
    <header className="mx-auto flex min-h-[6rem] w-full max-w-7xl items-center justify-between gap-5 px-5 py-5 lg:px-8 lg:py-6">
...
function TrustStrip() {
  ...
  return <section className="border-b border-border/70 bg-background"><div className="mx-auto flex max-w-7xl flex-wrap gap-x-8 gap-y-4 px-5 py-5 text-sm text-muted-foreground lg:px-8">{items.map(([Icon, label]) => <span key={label} className="flex items-center gap-2"><Icon className="size-4 text-primary" />{label}</span>)}</div></section>;
}
...
function Testimonials({ value }: { value: string }) { let items: Array<{ name: string; quote: string }> = []; try { items = JSON.parse(value); } catch { items = []; } return <section className="mx-auto max-w-7xl px-5 py-16 lg:px-8"><SectionIntro eyebrow="CUSTOMER STORIES" title="Built for confidence" text="A clearer workspace helps members stay focused on the work that matters." /><div className="mt-8 grid gap-4 md:grid-cols-2">{items.slice(0, 3).map((item) => <figure key={item.name} className="surface p-6"><blockquote className="text-sm leading-6 text-muted-foreground">“{item.quote}”</blockquote><figcaption className="mt-5 text-sm font-semibold">{item.name}</figcaption></figure>)}</div></section>; }
...
function FeatureSection() {
  ...
  return <section className="border-y border-border/70 bg-muted/20"><div className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-24"><SectionIntro eyebrow="FEATURES" title="Designed around the information you need" text="Keep an eye on daily activity without losing the context behind your account." /><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{features.map(([Icon, title, text]) => <div key={title as string} className="surface p-5"><Icon className="size-5 text-primary" /><h3 className="mt-5 text-sm font-semibold">{title as string}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{text as string}</p></div>)}</div></div></section>;
}

function PlansPreview({ plans }: { plans: typeof PLANS }) { return <section id="plans" className="mx-auto max-w-7xl px-5 py-20 lg:px-8 lg:py-28"><SectionIntro eyebrow="PLANS" title="Choose your plan" text="Review the plans currently available in your account system." /><div className="mt-10 grid gap-4 md:grid-cols-3">{plans.length ? plans.slice(0, 3).map((plan) => <div key={plan.id} className="surface flex flex-col p-6"><div className="flex items-start justify-between gap-4"><h3 className="font-semibold">{plan.name}</h3><Badge variant="secondary">Available</Badge></div><p className="mt-6 text-3xl font-semibold">{money(plan.price)}</p><p className="mt-1 text-sm text-muted-foreground">Lifetime access</p><div className="mt-6 space-y-3 border-t border-border/70 pt-4 text-sm text-muted-foreground"><p>{plan.dailyAdLimit} eligible tasks per day</p><p>{plan.referrerCommissionPct}% direct referral commission</p></div><Button asChild variant="outline" className="mt-6 w-full"><Link to="/plans">View Plan <ArrowRight className="size-4" /></Link></Button></div>) : <div className="surface p-8 text-sm text-muted-foreground md:col-span-3">No plans are currently available.</div>}</div></section>; }

function WhyAdNet() { return <section className="border-y border-border/70 bg-muted/20"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-20 lg:grid-cols-[.8fr_1.2fr] lg:px-8 lg:py-24"><SectionIntro eyebrow="WHY ADVERX" title="Built around clarity" text="A structured account experience makes it easier to understand what is available and what has already happened." /><div className="grid gap-3 sm:grid-cols-2">{["Clear task eligibility", "Visible reward history", "Wallet transaction history", "Structured withdrawal flow", "Protected account access"].map((item) => <div key={item} className="flex items-center gap-3 rounded-2xl border border-border/70 bg-background p-4 text-sm"><CheckCircle2 className="size-4 text-primary" />{item}</div>)}</div></div></section>; }

function Faq() {   const items: Array<[string, string]> = [["How do tasks work?", "Available tasks are shown in your workspace when your account has an eligible plan. Completion and daily limits are tracked by the system."], ["How are rewards calculated?", "Rewards are credited after a verified task is completed and are subject to the limits and reserve available to your plan."], ["When can I request a withdrawal?", "You can request a withdrawal when your account meets the current eligibility rules and has sufficient available wallet balance."], ["How do referrals work?", "Referral activity and eligible network rewards are tracked in your account workspace according to the active referral rules."], ["Are plans lifetime or time-limited?", "The available plan card identifies whether a plan provides lifetime access or a time-limited duration."], ["Where can I see my transaction history?", "Your wallet and transaction history are available after signing in to your account."]]; return <section id="faq" className="mx-auto max-w-3xl px-5 py-20 lg:py-28"><SectionIntro eyebrow="FAQ" title="Common questions" text="Straight answers about using the AdverX workspace." /><div className="mt-10 divide-y divide-border rounded-2xl border border-border/70 bg-card">{items.map(([question, answer]) => <details key={question} className="group p-5"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-medium"><span>{question}</span><ChevronDown className="size-4 shrink-0 text-muted-foreground transition-transform group-open:rotate-180" /></summary><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">{answer}</p></details>)}</div></section>; }
