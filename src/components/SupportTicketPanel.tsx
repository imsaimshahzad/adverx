...
export function SupportTicketPanel({ admin = false }: { admin?: boolean }) {
  ...
  const loadTickets = useCallback(async () => {
    ...
    if (error) toast.error(error.message); else setTickets(data ?? []);
    setLoading(false);
  }, [admin]);
