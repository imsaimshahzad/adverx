...
export function SupportTicketPanel({ admin = false }: { admin?: boolean }) {
  ...
  async function createTicket(): Promise<void> { const parsedSubject = subjectSchema.safeParse(subject); const parsedDescription = supportTextSchema.safeParse(description); if (!parsedSubject.success || !parsedDescription.success) { toast.error("Subject must be 3-180 characters and description must be 1-10,000 characters."); return; } const { data: auth } = await db.auth.getUser(); if (!auth.user) return; const { data, error } = await db.from("support_tickets").insert({ user_id: auth.user.id, subject: subject.trim(), category, description: description.trim(), message: description.trim(), status: "open", priority: "normal" }).select().single(); if (error) { toast.error(error.message); return; } await db.from("support_messages").insert({ ticket_id: data.id, sender_type: "user", message: description.trim() }); toast.success("Support ticket created."); setSubject(""); setDescription(""); setCreating(false); await loadTickets(); await openTicket(data); }
  async function sendReply(): Promise<void> { if (!selected || !uuidSchema.safeParse(selected.id).success || !supportTextSchema.safeParse(reply).success) { toast.error("Enter a valid reply."); return; } const { error } = await db.from("support_messages").insert({ ticket_id: selected.id, sender_type: admin ? "admin" : "user", message: reply.trim() }); if (error) { toast.error(error.message); return; } if (!admin) await db.from("support_tickets").update({ updated_at: new Date().toISOString() }).eq("id", selected.id); setReply(""); await openTicket(selected); await loadTickets(); }
